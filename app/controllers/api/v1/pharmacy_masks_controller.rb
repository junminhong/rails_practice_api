class Api::V1::PharmacyMasksController < ApplicationController
    def index
        show_all_pharmacy_masks if params[:choose] && params[:sort] && params[:pharmacy_id]
...
private
    ...
    def sort_pharmacy_masks_by_asc
        sort_label = params[:choose] == 0 ? "pharmacy_masks.name ASC" : "pharmacy_masks.price ASC"
        pharmacy_store_mask = PharmacyMask.where("pharmacy_id=?", params[:pharmacy_id]).order(sort_label)
    ...
    def sort_pharmacy_masks_by_desc
        sort_label = params[:choose] == "0" ? "pharmacy_masks.name DESC" : "pharmacy_masks.price DESC"
        pharmacy_store_mask = PharmacyMask.where("pharmacy_id=?", params[:pharmacy_id]).order(sort_label)
        all_pharmacy_store_masks = remove_created_and_updated_time_cols(pharmacy_store_mask)
        render json: all_pharmacy_store_masks
    end
